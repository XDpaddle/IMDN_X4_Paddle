...
def conv_layer(in_channels, out_channels, kernel_size, stride=1, dilation=1, groups=1, bias=True):
    ...
    return nn.Conv2D(in_channels, out_channels, kernel_size, stride, padding=padding,  dilation=dilation,
                     groups=groups)
...
def mean_channels(F):
    ...
    return spatial_sum / (F.shape[2] * F.shape[3])

def stdv_channels(F):
    assert(F.dim() == 4)
    F_mean = mean_channels(F)
    F_variance = (F - F_mean).pow(2).sum(3, keepdim=True).sum(2, keepdim=True) / (F.shape[2] * F.shape[3])
...
class IMDModule(nn.Layer):
    ...
    def forward(self, input):
        ...
        distilled_c1, remaining_c1 = paddle.split(out_c1, (self.distilled_channels, self.remaining_channels), axis=1)
        out_c2 = self.act(self.c2(remaining_c1))
        distilled_c2, remaining_c2 = paddle.split(out_c2, (self.distilled_channels, self.remaining_channels), axis=1)
        out_c3 = self.act(self.c3(remaining_c2))
        distilled_c3, remaining_c3 = paddle.split(out_c3, (self.distilled_channels, self.remaining_channels), axis=1)
        out_c4 = self.c4(remaining_c3)
        out = paddle.concat([distilled_c1, distilled_c2, distilled_c3, out_c4], axis=1)
        out_fused = self.c5(self.cca(out)) + input
        return out_fused

class IMDModule_speed(nn.Layer):
    def __init__(self, in_channels, distillation_rate=0.25):
...
class IMDModule_Large(nn.Layer):
    ...
    def forward(self, input):
        ...
        distilled_c1, remaining_c1 = paddle.split(out_c1, (self.distilled_channels, self.remaining_channels), axis=1) # 6, 18
        out_c2 = self.act(self.c2(remaining_c1))  #  18 --> 24
        distilled_c2, remaining_c2 = paddle.split(out_c2, (self.distilled_channels, self.remaining_channels), axis=1)  # 6, 18
        out_c3 = self.act(self.c3(remaining_c2))  # 18 --> 24
        distilled_c3, remaining_c3 = paddle.split(out_c3, (self.distilled_channels, self.remaining_channels), axis=1)  # 6, 18
        out_c4 = self.act(self.c4(remaining_c3))  # 18 --> 18
        distilled_c4, remaining_c4 = paddle.split(out_c4, (self.distilled_channels, self.remaining_channels-self.distilled_channels), axis=1)  # 6, 12
        out_c5 = self.act(self.c5(remaining_c4))  # 12 --> 12
        distilled_c5, remaining_c5 = paddle.split(out_c5, (self.distilled_channels, self.remaining_channels-self.distilled_channels*2), axis=1)  # 6, 6
        out_c6 = self.act(self.c6(remaining_c5))  # 6 --> 6

        out = paddle.concat([distilled_c1, distilled_c2, distilled_c3, distilled_c4, distilled_c5, out_c6], axis=1)
